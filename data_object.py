...
class DATA_OBJECT(object):
    def __init__(self,
        ...
        if one_hot:
            ...
            # Define the indexes.
            index = np.arange(self._num_samples) * num_classes              # np.arange(5)=[0,1,2,3,4]    np.arange(5)*3=[0,3,6,9,12]
...
def provide_data(mnist):
    ...
    validation = DATA_OBJECT(validation_images, validation_labels, num_classes=10, one_hot=True, dtype=np.float32, reshape=False)
    ...
    DataSetObject = collections.namedtuple('DataSetObject', ['train', 'validation', 'test'])  # data = DataSetObject(1,2,3)  data.train = 1   data.test = 3
